...
public:
	...
	{
	d:
		cout << "Enter day number in dd format : ";
		cin >> date[0] >> date[1];
		...
		date[2] = '-';
	m:
		cout << "Enter month number in mm format : ";
		cin >> date[3] >> date[4];
		int mon = int(date[3]) * 10 + int(date[4]);
		if (mon > 12)
		{
		...
		date[5] = '-';
	y:
		cout << "Enter year number in yyyy format : ";
		cin >> date[6] >> date[7] >> date[8] >> date[9];
		int years = int(date[6]) * 1000 + int(date[7]) * 100 + int(date[8]) * 10 + int(date[9]);
		if (years < 2021)
		{
			cout << "Enter valid year number.";
			goto y;
		}
		date[10] = '\0';
		return date;
